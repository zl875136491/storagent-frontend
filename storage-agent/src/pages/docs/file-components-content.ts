...
export function generateComponentGuideMarkdown(
  ...
  return [
    ...
    "",
    isTypeScript
      ? "将完整组件文件加入 React 项目后直接引入。组件覆盖空文件校验、动态分片上传、object_key 回填、元信息查询、服务点定位、当前节点与跨区域回退下载，并显示下载进度、速度和取消动作。生产环境请改为 App 后端签发能力令牌后再由浏览器直连数据面。"
      : "将客户端类文件加入 Python 项目后直接实例化。类对外提供带空文件校验和动态分片的上传、元信息、定位和下载方法；上传可返回 object_key 或完整 JSON，下载支持进度回调、取消事件并返回结果字典。该类应运行在服务端；若面向浏览器，请改用功能接口引导 v1 的能力令牌流程。",
    "",
    "## 安全约束",
    "",
    "- **控制台演示路径**：浏览器只发送当前用户已持有 APIKey 的对象引用；服务端验证归属、吊销状态、有效期和应用状态后执行操作，APIKey 明文不进入浏览器、URL 或日志。",
    "- **生产路径（v1）**：浏览器不得持有 `x-api-key`；分片上传 / 下载改用 App 后端签发的能力令牌 `token`，并由浏览器直连 Storagent 数据面，不要经业务后端中转文件字节；页面 Origin 须在 Storagent 控制台「应用管理 → 浏览器来源」中登记。",
    ...
    "- [ ] 生产接入时上传和下载由浏览器直连 Storagent 数据面，文件字节不经业务后端中转；页面 Origin 已在「应用管理 → 浏览器来源」登记。",
