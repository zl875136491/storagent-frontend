...
// 后端。新增版本时只需要：
// 1）在下面的 DOC_VERSIONS 里将新版本插入到最前面；
...
// Keep the newest registered version first so every shared selector opens on
// the highest API version when the URL does not specify `docv`.
export const DEFAULT_DOC_VERSION: DocVersion = DOC_VERSIONS[0].id
