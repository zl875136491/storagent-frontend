...
import {
  fetchAuditEventOptionsApi,
  fetchAuditEventsApi,
  type AuditEventItem,
  type AuditEventOptionsResponse,
...
function initialFilters(): FilterState {
  const end = new Date()
  return { start_at: datetimeInput(new Date(end.getTime() - 7 * 86400000)), end_at: datetimeInput(end) }
}

function EventResult({ success }: { success: boolean }) {
  return success ? (
    <span className="inline-flex items-center gap-1 text-emerald-700 dark:text-emerald-300"><CheckCircle2 className="h-3.5 w-3.5" aria-hidden />成功</span>
  ) : (
...
function AuditDetail({ event }: { event: AuditEventItem | null }) {
  ...
  if (!event) {
    return <Card className="flex h-full min-h-0 min-w-0 flex-col rounded-lg shadow-none"><CardContent className="flex h-full min-h-0 items-center justify-center p-6 text-center text-sm text-muted-foreground">选择一条审计事件查看详细记录。</CardContent></Card>
  }
  let detail = event.detail
  try { detail = JSON.stringify(JSON.parse(event.detail), null, 2) } catch { /* Audit detail can be plain text. */ }
  const copyResource = async () => {
    if (!event.resource) return
    const ok = await navigator.clipboard?.writeText(event.resource).then(() => true).catch(() => false)
    if (ok) { setCopied(true); window.setTimeout(() => setCopied(false), 1200) }
  }
  return <Card className="flex h-full min-h-0 min-w-0 flex-col rounded-lg shadow-none"><CardContent className="flex h-full min-h-0 flex-col p-5"><div className="flex shrink-0 items-start justify-between gap-3 border-b border-border/70 pb-4"><div className="min-w-0"><h2 className="text-base font-semibold text-foreground">审计事件详情</h2><p className="mt-1 truncate font-mono text-xs text-muted-foreground">{event.action_label || event.action}</p></div><EventResult success={event.success} /></div><dl className="mt-4 grid shrink-0 gap-4 sm:grid-cols-2"><div><dt className="text-[11px] text-muted-foreground">发生时间</dt><dd className="mt-1 text-xs">{formatDateTime(event.created_at)}</dd></div><div><dt className="text-[11px] text-muted-foreground">区域</dt><dd className="mt-1 text-xs">{event.region || "-"}</dd></div><div><dt className="text-[11px] text-muted-foreground">操作者</dt><dd className="mt-1 text-xs">{event.actor || "-"}</dd></div><div><dt className="text-[11px] text-muted-foreground">资源</dt><dd className="mt-1 flex items-center gap-1 text-xs"><span className="min-w-0 break-all">{event.resource || "-"}</span>{event.resource ? <Button type="button" variant="ghost" size="icon-sm" title="复制资源标识" onClick={() => void copyResource()}><Copy className={cn("h-3.5 w-3.5", copied && "text-emerald-600")} aria-hidden /></Button> : null}</dd></div></dl><div className="mt-5 flex min-h-0 flex-1 flex-col"><div className="mb-2 shrink-0 text-sm font-semibold text-foreground">详情</div><pre className="min-h-0 flex-1 overflow-auto rounded-md border border-border/70 bg-muted/25 p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap break-words">{detail || "无额外详情"}</pre></div></CardContent></Card>
}
