...
function VerificationDetailPanel({ detail }: { detail: VerificationDetail | null }) {
  return (
    <Card className="h-full min-h-[42rem] rounded-lg shadow-none" aria-live="polite">
...
function StructuredDiagnosticLog({ run }: { run: DiagnosticRun }) {
  ...
  return <pre className="max-h-72 overflow-auto rounded-md border border-border/70 bg-muted/20 p-3 font-mono text-[11px] leading-relaxed text-foreground">{JSON.stringify(report, null, 2)}</pre>
}
...
function DiagnosticWorkspace({ version, accessToken }: { version: "v1" | "v2"; accessToken?: string }) {
  ...
  return (
    <section className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(21rem,0.75fr)_minmax(0,1.25fr)]">
      ...
      <Card className="min-h-[42rem] rounded-lg shadow-none"><CardContent className="flex h-full min-h-0 flex-col p-5">
  ...
  return (
    <div className="mx-auto flex h-full min-h-[680px] max-w-8xl flex-col pb-10">
      ...
      {view === "diagnostics" ? <DiagnosticWorkspace version={version} accessToken={accessToken ?? undefined} /> : <>
        ...
        <section className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)]"><div className="min-w-0"><StorageApiVerificationPanel version={version} baseURL={base} apiKeyId={apiKeyId} accessToken={accessToken ?? undefined} onDetailSelect={setSelectedDetail} /></div><div className="min-h-0"><VerificationDetailPanel detail={selectedDetail} /></div></section>
      </>}
    </div>
  )
}
