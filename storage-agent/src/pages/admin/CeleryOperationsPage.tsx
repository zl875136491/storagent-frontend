        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={`${item.source}-${item.id}-${item.status}`}>
              ...
              <TableCell className="min-w-44 align-top text-xs"><div className="truncate" title={item.worker}>{item.worker || "-"}</div><div className="mt-1 text-muted-foreground">执行区 {item.region || "未上报区域"}</div>{item.origin_region ? <div className="mt-1 text-muted-foreground">来源区 {item.origin_region}{item.task_protocol ? ` · v${item.task_protocol}` : ""}</div> : null}</TableCell>
  ...
  return (
    <div className="mx-auto max-w-8xl pb-4">
      <div className="mb-4 flex shrink-0 flex-wrap items-center justify-between gap-3">
        <div><div className="flex items-center gap-2"><Workflow className="h-5 w-5 text-sky-600 dark:text-sky-300" aria-hidden /><h1 className="text-lg font-semibold text-foreground">Celery 运维</h1></div><p className="mt-1 text-xs text-muted-foreground">区域队列、Worker、任务执行与历史记录。</p></div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)]">
        ...
        <div className="flex flex-col gap-4">
          <section className="overflow-hidden rounded-lg border border-border/80 bg-background"><div className="border-b border-border/70 px-4 py-3"><h2 className="text-sm font-semibold">Worker 状态</h2></div><div className="max-h-[23rem] overflow-auto"><Table><TableHeader><TableRow><TableHead>Worker</TableHead><TableHead>状态</TableHead><TableHead>负载</TableHead></TableRow></TableHeader><TableBody>{overview?.workers.length ? overview.workers.map((item) => <TableRow key={item.name}><TableCell className="min-w-48"><div className="truncate font-mono text-[11px]" title={item.name}>{item.name}</div><div className="mt-1 text-[11px] text-muted-foreground">{item.region || "未上报区域"} · {formatAge(item.heartbeat_age_seconds)}</div><div className="mt-1 truncate font-mono text-[10px] text-muted-foreground" title={item.queue}>{item.queue || "未上报队列"} · v{item.task_protocol || "-"} · {item.beat_enabled ? "参与 Beat" : "仅 Worker"}</div></TableCell><TableCell><WorkerBadge status={item.status} /></TableCell><TableCell className="text-xs"><div>执行 {item.active_count} / 待取 {item.reserved_count}</div><div className="mt-1 text-muted-foreground">并发 {item.concurrency ?? "-"} · 已处理 {item.processed_count}</div></TableCell></TableRow>) : <TableRow><TableCell colSpan={3} className="h-24 text-center text-sm text-muted-foreground">未发现 worker。</TableCell></TableRow>}</TableBody></Table></div></section>
          <section className="overflow-hidden rounded-lg border border-border/80 bg-background"><div className="border-b border-border/70 px-4 py-3"><h2 className="text-sm font-semibold">任务队列</h2></div><div className="max-h-[18rem] overflow-auto"><Table><TableHeader><TableRow><TableHead>队列</TableHead><TableHead>积压</TableHead><TableHead>Worker</TableHead><TableHead>路由</TableHead></TableRow></TableHeader><TableBody>{overview?.queues.length ? overview.queues.map((item) => <TableRow key={item.name}><TableCell className="font-mono text-xs">{item.name}</TableCell><TableCell className="font-medium">{item.pending_count.toLocaleString("zh-CN")}</TableCell><TableCell className="text-xs">{item.worker_count}</TableCell><TableCell className="max-w-36 truncate font-mono text-[10px] text-muted-foreground" title={item.routing_keys.join(", ")}>{item.routing_keys.join(", ")}</TableCell></TableRow>) : <TableRow><TableCell colSpan={4} className="h-24 text-center text-sm text-muted-foreground">未读取到队列信息。</TableCell></TableRow>}</TableBody></Table></div></section>
          <section className="overflow-hidden rounded-lg border border-border/80 bg-background"><div className="border-b border-border/70 px-4 py-3"><h2 className="text-sm font-semibold">Beat 领导者</h2></div><div className="max-h-40 overflow-auto"><Table><TableHeader><TableRow><TableHead>区域协议</TableHead><TableHead>状态</TableHead></TableRow></TableHeader><TableBody>{overview?.beat_leaders.length ? overview.beat_leaders.map((item) => <TableRow key={item.key}><TableCell className="min-w-48"><div className="truncate font-mono text-[10px]" title={item.key}>{item.key}</div><div className="mt-1 truncate text-[10px] text-muted-foreground" title={item.owner}>{item.owner || "-"}</div></TableCell><TableCell className="text-xs">{item.active ? <span className="text-emerald-700 dark:text-emerald-300">持有中</span> : <span className="text-muted-foreground">已过期</span>}</TableCell></TableRow>) : <TableRow><TableCell colSpan={2} className="h-20 text-center text-sm text-muted-foreground">尚未发现 Beat 租约。</TableCell></TableRow>}</TableBody></Table></div></section>
        </div>
      </div>
