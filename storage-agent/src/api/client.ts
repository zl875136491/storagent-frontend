...
async function ensureFailoverBackend(): Promise<string | null> {
  if (!failoverInFlight) {
    failoverInFlight = (async () => {
      const next = await failoverToAlternateBackend(
...
export type StorageOperationHealth =
  "healthy" | "syncing" | "degraded" | "critical" | "unreachable";
...
export interface EtcdOperationResponse {
  kind: string;
  status: string;
  message: string;
...
export interface EtcdTrendPoint {
  checked_at: string;
  status: EtcdStatus;
  reachable_endpoint_count: number;
  configured_endpoint_count: number;
...
export interface EtcdTrendResponse {
  data: EtcdTrendPoint[];
}

export interface EtcdEventItem {
  kind: string;
  status: string;
  actor: string;
  endpoint: string;
...
export interface EtcdEventListResponse {
  data: EtcdEventItem[];
}

export interface EtcdRevisionOption {
  revision: number;
  label: string;
}

export interface EtcdRevisionOptionsResponse {
  current_revision: number;
  options: EtcdRevisionOption[];
}

export type EtcdTaskStatus = "queued" | "running" | "succeeded" | "failed";

export interface EtcdTaskResponse {
  id: string;
  kind: string;
  status: EtcdTaskStatus;
  actor: string;
  message: string;
...
async function loginByCodeAtBackend(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const detail =
        typeof parsed?.data === "string" ? parsed.data : parsed?.msg;
...
export async function loginByCodeAcrossBackends(
  ...
  const unique = [
    ...new Set(
      candidates.map((item) => item.trim().replace(/\/$/, "")).filter(Boolean),
    ),
  ];
  if (unique.length === 0) {
  ...
  try {
    ...
    if (error instanceof AggregateError) {
      const detail = error.errors.find((item) => item instanceof Error) as
        Error | undefined;
...
export async function refreshTokenApi(
  ...
  return apiPost<{ refresh_token: string }, TokenResponse>(
    "/api/v1/auth/refresh",
    {
...
export async function createRegionApi(
  ...
  return apiPost<RegionCreateRequest, Region>(
    "/api/v1/public/region",
    payload,
    accessToken,
  );
}
...
export async function fetchMinioServersApi(
  ...
  return apiGet<MinioServerListResponse>(
...
export async function createAdminObjectDownloadLinkApi(
  ...
  return apiPost<
    AdminObjectDownloadLinkRequest,
    AdminObjectDownloadLinkResponse
  >(
...
export async function fetchClusterHealthOperationsApi(
  ...
  return apiGet<ClusterHealthResponse>(
    "/api/v1/storage/operations/clusters",
    accessToken,
  );
}
...
export async function fetchEtcdTrendApi(
  accessToken?: string,
): Promise<EtcdTrendResponse> {
  return apiGet<EtcdTrendResponse>(
    "/api/v1/storage/operations/etcd/trend",
    accessToken,
  );
}

export async function fetchEtcdEventsApi(
  accessToken?: string,
): Promise<EtcdEventListResponse> {
  return apiGet<EtcdEventListResponse>(
    "/api/v1/storage/operations/etcd/events",
    accessToken,
  );
}

export async function fetchEtcdRevisionOptionsApi(
  accessToken?: string,
): Promise<EtcdRevisionOptionsResponse> {
  return apiGet<EtcdRevisionOptionsResponse>(
    "/api/v1/storage/operations/etcd/revision-options",
    accessToken,
  );
}

export async function createEtcdTaskApi(
  payload: {
    kind: "keyspace" | "compact" | "defrag" | "alarm-disarm";
    revision?: number;
  },
  accessToken?: string,
): Promise<EtcdTaskResponse> {
  return apiPost<typeof payload, EtcdTaskResponse>(
    "/api/v1/storage/operations/etcd/tasks",
    payload,
    accessToken,
  );
}

export async function fetchEtcdTaskApi(
  taskId: string,
  accessToken?: string,
): Promise<EtcdTaskResponse> {
  return apiGet<EtcdTaskResponse>(
    "/api/v1/storage/operations/etcd/tasks/" + encodeURIComponent(taskId),
    accessToken,
  );
}

export async function fetchEtcdTasksApi(
  accessToken?: string,
): Promise<EtcdTaskListResponse> {
  return apiGet<EtcdTaskListResponse>(
    "/api/v1/storage/operations/etcd/tasks?limit=50",
    accessToken,
  );
}

export async function inspectEtcdKeyspaceApi(
  accessToken?: string,
): Promise<EtcdOperationResponse> {
  return apiGet<EtcdOperationResponse>(
    "/api/v1/storage/operations/etcd/keyspace",
    accessToken,
  );
}

export async function runEtcdOperationApi(
  operation: "compact" | "defrag" | "alarm-disarm",
  payload: { confirm: boolean; revision?: number },
  accessToken?: string,
): Promise<EtcdOperationResponse> {
  return apiPost<typeof payload, EtcdOperationResponse>(
    "/api/v1/storage/operations/etcd/" + operation,
    payload,
    accessToken,
  );
}

export async function downloadEtcdSnapshotApi(
  accessToken?: string,
): Promise<Blob> {
  const response = await authorizedFetch(
    "/api/v1/storage/operations/etcd/snapshot",
    { method: "GET" },
    accessToken,
  );
  if (!response.ok) {
    const text = await response.text().catch(() => "");
    throw new Error(text || "快照下载失败，状态码 " + response.status);
  }
  return response.blob();
}

export async function stageEtcdRestoreApi(
  file: File,
  accessToken?: string,
): Promise<EtcdOperationResponse> {
  const body = new FormData();
  body.append("file", file);
  return apiPost<FormData, EtcdOperationResponse>(
    "/api/v1/storage/operations/etcd/restore",
    body,
    accessToken,
  );
}
...
export async function fetchBucketReplicatesApi(
  ...
  return apiGet<BucketReplicatesResponse>(
...
export async function postBucketGraphNodePosition(
  ...
  return apiPost<
    BucketGraphNodePositionPayload,
    BucketGraphNodePositionPayload
  >("/api/v1/graph/bucket-node-position", payload, accessToken);
}
...
export async function postBucketGraphEdgePosition(
  ...
  return apiPost<
    BucketGraphEdgePositionPayload,
    BucketGraphEdgePositionPayload
  >("/api/v1/graph/bucket-edge-position", payload, accessToken);
}
...
export async function fetchQuotaAlertRuleApi(
  accessToken?: string,
...
export async function fetchExpansionRequestsApi(
  accessToken?: string,
): Promise<ExpansionRequestListResponse> {
  return apiGet<ExpansionRequestListResponse>(
    "/api/v1/public/application/expansion-requests",
    accessToken,
  );
}
...
export async function fetchDiagnosticRunsApi(
  accessToken?: string,
): Promise<{ data: DiagnosticRun[] }> {
  return apiGet<{ data: DiagnosticRun[] }>(
...
export async function fetchCapacityPlanningApi(
  accessToken?: string,
...
export type ApplicationApprovalSseStatus =
  "running" | "ok" | "failed" | "success" | "skipped";
...
function parseSseEventLine(line: string): ApplicationApprovalSseEvent | null {
  ...
  try {
    ...
    return {
      step: o.step,
      server_name:
        serverName === null || typeof serverName === "string"
          ? serverName
          : null,
...
export async function fetchDemoApiKeysApi(
  accessToken?: string,
): Promise<DemoAPIKeyListResponse> {
...
export async function fetchEnabledApplicationsApi(
  ...
  return apiGet<SimpleApplicationListResponse>(
    "/api/v1/public/application/enabled",
    accessToken,
  );
}
...
export async function createApiKeyApi(
  ...
  return apiPost<APIKeyCreateRequest, APIKey>(
...
export async function revokeApiKeyApi(
  ...
  return apiDelete<{ message: string }>(
    `/api/v1/public/api-key/${apiKeyId}`,
    accessToken,
  );
}
...
export async function fetchPublicEndpointsApi(
  ...
  return apiGet<PublicEndpointsResponse>(
...
export async function fetchAuditEventOptionsApi(
  ...
  return apiGet<AuditEventOptionsResponse>(
 ...
 */
export function extractCrossRegionLocations(
  errorText: string,
): ObjectLocationItem[] | null {
  const body = parseApiErrorBody(errorText);
  if (!body) return null;
  const data = body.data as ObjectNotFoundLocalData | undefined;
  if (
    !data ||
    !Array.isArray(data.available_at) ||
    data.available_at.length === 0
  ) {
    return null;
  }
