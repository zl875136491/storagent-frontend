...
export function generateWorkflowMarkdown(version: string): string {
  ...
  lines.push("## 固定边界", "", "- App 后端使用 Python，负责业务鉴权、控制面调用和能力令牌签发。", "- App 前端使用 TypeScript，负责以 token 直连数据面上传或下载；文件字节不经 App 后端中转。", "- 前端直连 Storagent 前，须在控制台「应用管理 → 浏览器来源」登记当前页面 Origin（形如 `https://app.example.com`）。", "- 其他技术栈由开发者依据同一调用边界自行适配。", "")
  return lines.join("\n")
}
