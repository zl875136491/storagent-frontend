...
export function generateWorkflowMarkdown(version: string): string {
  const lines = ["# Storagent 调用时序总览（" + version + "）", "", "本文件对应控制台快速开始中的调用关系图。", "", "## 网关入口", "", "Storagent 的页面与 API 都通过 Nginx 访问。根路径只提供前端；每一个 API 地址都必须以 `/server/{region}` 为基址，Nginx 转发时会移除该区域前缀。", "", "| 环境 | 网关基址 | 路由结果 |", "| --- | --- | --- |", "| 生产（北京示例） | `http://stor.1oa.com.cn/server/bj` | 北京后端 |", "| NUC 测试默认 | `http://10.32.12.110/server/local` | 后端 A |", "| NUC 测试 A | `http://10.32.12.110/server/nuc-a` | 后端 A（nuc-docker-a） |", "| NUC 测试 B | `http://10.32.12.110/server/nuc-b` | 后端 B（nuc-docker-b） |", ""]
