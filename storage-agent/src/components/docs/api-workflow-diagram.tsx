/* eslint-disable react-refresh/only-export-components -- 导出 Markdown 生成器与可视图共享同一份调用定义，避免图文漂移。 */
import { useState } from "react"
import { Cloud, FileDown, FileUp, Laptop, Maximize2, Server, X } from "lucide-react"

import { cn } from "@/lib/utils"

type Actor = "browser" | "app" | "storagent"
type FlowKind = "upload" | "download"
...
const FLOW_TITLES: Record<FlowKind, string> = { upload: "上传调用时序", download: "下载调用时序" }
...
function FlowCanvas({ flow, selected, onSelect, fullscreen = false }: { flow: FlowLink[]; selected: string; onSelect: (id: string) => void; fullscreen?: boolean }) {
  ...
  const height = fullscreen ? 450 : 380
...
export function ApiWorkflowDiagram() {
  const [fullscreen, setFullscreen] = useState<FlowKind | null>(null)
  return <div className="mt-4 space-y-4"><FlowCard kind="upload" onExpand={() => setFullscreen("upload")} /><FlowCard kind="download" onExpand={() => setFullscreen("download")} />{fullscreen ? <FullscreenFlow kind={fullscreen} onClose={() => setFullscreen(null)} /> : null}</div>
}
