...
import { normalizePublicApiBase, probePublicEndpointTest } from "@/api/backendResolver"
...
function pickDisplayItems(items: PublicEndpointItem[]): PublicEndpointItem[] {
  ...
  for (const it of items) {
    const k = normalizePublicApiBase(it.endpoint)
...
export function GuideEndpointsProvider({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRawItems(items)
      const bases = pickDisplayItems(items).map((it) => normalizePublicApiBase(it.endpoint))
